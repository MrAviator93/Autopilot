...
inline scalar CDynamics::calcReynoldsNumber( const scalar density,
...
inline scalar CDynamics::calcHorizontalTailVolumeRatio( const scalar horizontalTailArea,
														const scalar lt,
														const scalar MAC,
...
inline scalar CDynamics::calcVerticalTailVolumeRatio( const scalar verticalTailArea,
													  const scalar lv,
...
inline scalar CDynamics::estimateDragDueToAoA( const scalar coeficientOfLift,
											   const scalar oswaldEfficiencyNumber,
											   const scalar wingAspectRatio,
											   const scalar liftDueToAngleOfAttack )
{
	// ( (2 * Cl) / PI() * e * AR) * Clα
	return ( ( ( 2 * coeficientOfLift ) / ( PI * oswaldEfficiencyNumber * wingAspectRatio ) ) *
			 liftDueToAngleOfAttack );
}

inline scalar CDynamics::estimateChangeInDownwashDueToAoA( const scalar liftDuetoAngleOfAttack,
														   const scalar wingAspectRatio )
{
	// Nelson, pg.125
