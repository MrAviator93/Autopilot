...
CSerialPort::CSerialPort(const char *pPortName, BoudRate boudRate) {
  this->m_pHandle =
      CreateFileA(static_cast<LPCSTR>(pPortName), GENERIC_READ | GENERIC_WRITE,
...
bool CSerialPort::writeSerialPort(char *pBuffer, uint32 bufferSize) {
  ...
  if (!WriteFile(this->m_pHandle, (void *)pBuffer, bufferSize, &bytesSend, 0)) {
    ClearCommError(this->m_pHandle, (DWORD *)&this->m_errors,
                   (LPCOMSTAT) & this->m_pStatus);
    return false;
  }
...
bool CSerialPort::isConnected() {
  if (!ClearCommError(this->m_pHandle, (DWORD *)&this->m_errors,
                      (LPCOMSTAT) & this->m_pStatus)) {
    m_initialized = false;
  }
